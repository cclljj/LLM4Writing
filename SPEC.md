...
- `1~5`：定位、架構、資料模型、權限、步驟引擎
- `6`：前端頁面規格（student / teacher）
- `7`：API 契約
- `8`：Prompt 配置
- `9~13`：預設資料、不變量、限制、維運、重建指引
...
- `getSessionWithMeta` 以 React `cache()` 包裝，同一 request 生命週期內多次呼叫同一 sessionId 只查詢一次 DB（#232）
...
- 程式行為：
  ...
  - 在學生流程的 LLM 呼叫中，已實作「跨步驟累積上下文」：
    - 會帶入作文題目（`activityTitle`）
    - Step1~Step4（小組階段）：帶入同組全員於 Step1~目前步驟的歷史互動（`student` / `ai` / 必要 `system`）
    - Step5 之後（個人階段）：帶入「Step1~4 小組共同歷史」+「Step5~目前步驟僅該學生本人歷史」
    - 個資隔離：僅允許同組範圍資料；Step5+ 不得混入他人在 Step5+ 的個人訊息
    - 長度控制：以上下文「最近訊息數 + 字數上限」雙重裁切（避免 prompt 過長）
    - 套用於有實際 LLM 呼叫的學生步驟路徑（含 Step1/2/3/4 與個人階段相關呼叫）
  - 若缺任一：使用內建 stub 回覆文字（讓 UI 流程可跑，但不代表真實 LLM）
  - 若遠端 LLM 回傳格式不符或請求失敗：先進行最多 3 次短暫退避重試；仍失敗時自動 fallback 為內建回覆，不中斷課程流程
  - 若 OpenAI-compatible 回覆的 `finish_reason` 顯示為長度/token 限制截斷，且該呼叫不是 Step1/2 結構化 JSON 題目輸出，系統會自動要求模型從中斷處續寫並合併文字，避免把半截回覆存成完整 AI 訊息。
...
- 模組（#231 拆分後由三個檔案組成）：
  - `src/lib/activity-store.ts`：Domain 狀態單例（users / essays / openClasses / groups / courseStatus）、hydrate/flush、所有 CRUD 與查詢函式
  - `src/lib/genre-resolver.ts`：依文體解析結構樹範本路徑、讀取範本並替換題目佔位符（`resolveStructureTreeTemplate`）
  - `src/lib/prompt-config.ts`：步驟開場白載入、`resolvePromptConfigForActivity`
  - `src/lib/mock-data.ts`：re-export barrel，保留舊 import 路徑向下相容
...
## 3.3 OpenClassTask / OpenClassView（在 activity-store 中）
...
- 對於此類條目：
  ...
  - `1-3-1`/`1-4-1`/`2-1-1` 等首題若 `nextQuestion` 缺失，需使用短 fallback 問句；不得回退成 subStepPrompts 原文
  - 前端互動區需加防漏保護：若系統訊息看起來是指示型 prompt（規則段落/長文），顯示替代文案而非原文
  - `nextQuestion` 缺失時，系統需先重試生成（最多 3 次）；三次皆缺失才允許使用短、安全 fallback 問句
  - 此重試規則至少適用於：`1-3-x`、`1-4-x`、`2-1-x`、`2-2`、`2-3`

## 5.2 學生回答品質檢查（送出前驗證）

- 在 `sendStudentMessage` 寫入有效訊息前，會先做基本品質檢查。
- 目標：允許學生答錯，但拒絕明顯敷衍、惡搞或不符合最低要求的回覆。
- 目前檢查項目：
  - 過短內容（不足以表達完整想法）
  - 明顯敷衍詞（如「不知道 / 隨便 / 沒意見」等）
  - 直接貼題（答案與當前系統題目高度相同）
...
- Step1/2 採 group gate 機制，以「目前子步驟（含子題）key」計算回覆完成度：
  - 例：`1-3-1`、`1-4-2`、`2-1-3`
- 學生在自己尚未完成當前子步驟回答前：
  - 不可看到同組其他同學在該子步驟的 student 訊息
- 當最後一位同學送出當前子步驟回答後：
...
- Step1/2 在全員送出後不得因遠端 LLM 失敗而停在「等待遠端 AI」：
  - Step1/2 的單次遠端 LLM 嘗試需使用較短 timeout，避免課堂互動長時間卡住。
  - 若遠端 LLM 失敗、逾時或沒有產生可用 `nextQuestion`，後端需使用 `subStepPrompts_fallbacks` / 內建安全 fallback 推進到下一子步驟。
  - 若舊 session 已保存為「目前 gate 全員完成」但沒有下一則 AI/system 題目，學生端輪詢 `/api/session/[sessionId]` 時需在安全等待時間（目前預設 45 秒）後自動補推進，避免永久卡住。
  - 自動補推進需涵蓋 `1-3-3 -> 1-4-1`、`1-4-3 -> 1-5`、`2-1-3 -> 2-2` 等子題邊界，不可只處理子題中段。

### 學生可讀文字淨化規則

- 所有會直接顯示在學生畫面的 AI 文字都必須先經「學生可讀文字淨化」，避免出現 JSON/code fence/欄位名等開發格式殘留。
- 目前至少套用於：Step1/2 回饋、Step3/4 一般互動、Step6 修改建議、Step7 分析回饋、Step10 總結報告。
- 淨化規則需移除以下常見洩漏：
  - Markdown code fence（如 ```` ```json ````）
  - 結構化欄位包裝（如 `{"feedback":...,"nextQuestion":...}`）
  - 只保留對學生可讀、自然語句的內容。
...
- `src/lib/llm-openai-response.ts`：OpenAI-compatible 回覆文字與 `finish_reason` 解析、長度截斷偵測。
...
- `src/lib/learning-diagnostics.ts`：教師端進階卡關偵測、回答品質拒絕訊號累計、Step3/Step6 artifact 診斷。
- `src/lib/student-next-action.ts`：學生端「下一步該做什麼」微引導判斷。
- `src/lib/outline-utils.ts`：Mermaid 結構樹解析共用函式（`extractMermaidText`、`fromMermaid`、`buildOutlinePreview`）與型別（`OutlineNode`、`OutlinePreview`）；三個前端頁面共用，任何解析 bug 只需修一處。
- `src/lib/api-helpers.ts`：student step API 路由共用驗證骨架（`requireStudentInSession`），統一處理 role 驗證 → getSession → participant 驗證 → markUserOnline；6 支 step route 共用。
- `app/_components/OutlineSvg.tsx`：共用結構樹 SVG React 元件，接受 `mermaidText`、`label?`、`compact?` props；teacher 端預設全尺寸，student/history 端使用 `compact` 精簡樣式。
- `src/lib/genre-resolver.ts`：依文體查找結構樹範本路徑並讀取，`resolveStructureTreeTemplate(genre, essayTitle)` 為唯一公開 API。（#231 從 mock-data 拆出）
- `src/lib/activity-store.ts`：Domain 狀態單例與所有 CRUD，包含 `hydrateDomainState`、`flushDomainState`、`findActivity`、`getAllActivities` 等；是系統核心 domain layer。（#231 從 mock-data 拆出）
- `src/lib/prompt-config.ts`：載入步驟開場白文字（`src/config/step-opening/*.md`）、組裝 `resolvePromptConfigForActivity`、`loadActivityWithConfig`（一次取得 activity + promptConfig，避免雙重呼叫）；依賴 `activity-store.findActivity`。（#231 從 mock-data 拆出，#233 新增 helper）
...
- 前端需使用共用錯誤轉譯 helper（`src/lib/error-messages.ts`）（#272），將 API error code（例如 `xxx_failed`、`invalid_step`、`session_not_found`）轉為學生與教師可理解的中文提示。
- 錯誤提示需包含可執行下一步，例如「請稍後再試」「請重新整理頁面」「請確認課程是否已開始」「請聯繫教師或系統管理員」。
- 若 API 已回傳自然語言驗證訊息或 `hint`，前端需保留該提示並附加「建議修改」；不得把品質檢查回饋硬轉成籠統錯誤。
- 未知技術錯誤需使用穩定 fallback 文案，不得直接顯示裸露技術字串。
...
6.1 學生首頁「自己參與過的課程清單」中，「查詢紀錄」按鈕需置於課程文字資訊右側，按鈕寬度採精簡貼齊文字。
6.2 已參與課程清單中的「最近步驟」需以該學生個人步驟（`personalSteps[username]`）為準，不得回退為全班步驟顯示。
6.3 `/student/history/[activityId]` 需沿用課程首頁卡片風格，至少包含課程內容卡與歷史步驟互動內容；互動內容需使用 HTML 格式渲染。
6.4 `/student/history/[activityId]` 的每個歷史步驟需以獨立卡片呈現，提供展開/閉合（預設閉合）。
6.5 `/student/history/[activityId]` 在 Step3 與 Step4 需分別顯示結構樹圖片（Step3 為提交快照、Step4 為修正後版本）。

### 6.2.2 進入 session 後通用規則
7. 進入 session 後顯示課程資訊卡，固定為四行：
  - 第一行：題目名稱（粗體、放大）
  - 第二行：文體、時長
  - 第三行：班級、組別
  - 第四行：組員名單
7.1 題目卡需在上述資訊後方顯示「引導說明」與「補充資料」，並以 HTML 呈現保留原始換行與段落。
7.2 Step1 互動內容不顯示引導說明與補充資料，僅顯示步驟提問與互動訊息；`stepPrompts["1"]` 屬內部 prompt，不得寫入學生可見對話紀錄。
8. 一旦進入 session，學生頁會切換到課程內容視圖（提供「返回學生端課程首頁」按鈕）。
8.1 學生端不提供「返回上一層」按鈕，避免與課程首頁返回入口重複。
8.2 學生端頂部名片（「學生端課程首頁」標題、學生身分 badge、登出按鈕）需由 `/student` 區域共用 layout 提供，並固定顯示於所有學生端頁面最上方（至少含 `/student` 與 `/student/history/[activityId]`）。
...
13. 步驟名片介紹詞僅在 Step `1/2/3/4/6/8/9` 顯示；Step `5/7` 不顯示。介紹詞內容需讀取 `src/config/step-opening/{step}.md`，並以 HTML 呈現保留 Markdown 應有格式（至少含標題、粗體/斜體、清單、引用、段落換行）。原先 `任務：xxx / Session: xxx` 顯示需移除。
13.1 小組對話與個人進度紀錄不得寫入 internal system prompt；紀錄僅保留學生可理解的提問與互動內容。
...
15.0 學生端需定期同步課程狀態（in_progress / paused / ended / not_started）；當老師切換課程狀態時，學生端不得要求 reload，需即時反映並同步調整是否可互動/提交。
15.0.1 學生端 polling 採單一 `setInterval` 迴圈：無 session 時每 15s 呼叫 `refreshOverview()`；有 session 時每 5s 拉 `/api/session/{id}`（含 ETag/304 節流），每 3 個 tick（≈15s）才額外呼叫 `refreshActivityStatuses()`。禁止同時存在兩支重疊的 polling interval。
...
18.1 進入 session 後需顯示 Step1~Step10 學習進度軌，依學生個人目前步驟（`personalSteps[user] ?? session.currentStep`）標示「已完成 / 進行中 / 待開始」。
18.2 Step1/2/4 小組互動階段需顯示「小組等待狀態」卡片，至少包含目前題目或 gate key、已完成人數 / 組員總數、尚未完成組員，以及等待 AI/老師推進提示。
18.3 進度軌下方需顯示「下一步該做什麼」微引導卡片，由 `buildStudentNextAction` 依目前步驟與狀態產生具體行動文案，例如：
  - 可回答時：提示「請先回答目前系統提問」
  - 已完成小組 gate 時：提示正在等待哪些同組同學或等待 AI/老師推進
  - Step3：提示編輯結構樹並按「完成結構樹」
  - Step4：提示查看同學結構樹、修正自己的結構樹並按「確認完成此步驟」
  - Step6/8：提示先儲存文章、初稿字數偏低時先完成開頭與理由段
  - Step9：提示四題反思需全部填完後一次送出
18.4 下一步提示需以任務卡呈現（#269）：每一步只突出一個主要行動（例如「回答這一題」「等待組員」「完成結構樹」「送出初稿」「送出最終稿」），並以狀態 pill 標示「輪到你 / 等待中 / 已保存 / 已完成」等狀態；卡片可提供一句輔助說明，但不得同時突出多個競爭性主要操作。

### 6.2.3 Step3（生成論點）
19. Step3 的「文章結構樹」需支援視覺化節點編輯：
  - 初始預設樹需依課程文體決定：透過 `src/config/structure-tree.json` 取得對應 `src/config/structure-tree/*.map`（Mermaid）作為學生可編修的起始結構
  - 預設樹最上層若含「作文題目」，需替換為當前課程文章題目
  - 若學生載入時的結構樹為空樹（例如僅單一節點），需自動套用對應文體的預設結構樹
  - 初始僅一個根節點
  - 第一層與第二層為固定架構，不可改名、拖曳或刪除
  - 第一層節點不顯示 `➕` 或 `➖`
  - 第二層節點只顯示 `➕`，可新增下一層，但不顯示 `➖`
  - 第三層以下節點可用 `➕` 新增下一層子節點並自動建立連線
  - 第三層以下且無子節點者可用 `➖` 刪除節點
  - 第三層以下節點可雙擊或長按直接編輯節點文字，也可拖曳調整位置與層次（連線同步更新）
  - 當結構樹超出可視範圍時，編輯區需提供水平與垂直捲軸（scroll bar）以支援左右/上下檢視
  - Step3 結構樹採自動儲存：
    - 新增/刪除節點後需自動儲存到 `outline`
    - 文字編輯於按 `Enter` 或點擊節點外部（離焦）時視為完成並自動儲存
    - Step3 不提供「儲存變更」按鈕
  - 若 `outline` 已有 Mermaid 存檔，重新載入或重開編輯器時需優先還原該內容，不可重置為預設單一節點
  - Auto-save 後若 session polling 暫時回傳舊版 `outline`，前端不得用舊版覆蓋剛完成的本地編輯；需等 server outline 追上本地剛儲存版本後才重新允許從 session 同步
  ...
  - 編輯器上方需顯示 Step3 工具提示，說明第一/第二層固定、第三層以下新增/刪除/雙擊/長按編輯、Enter/離焦儲存等操作
  - 編輯器需顯示輕量儲存狀態，例如「結構樹已同步」「正在編輯節點」「有未儲存變更」「已完成送出」
20. Step3 版面規則：互動內容區需顯示在結構樹編輯器上方。
20.1 Step3 需直接顯示結構樹編輯畫面，不需額外「文章結構樹」開啟按鈕；不得因輪詢同步自動關閉編輯區。
...
28.1 前序步驟回顧卡片在「閉合」狀態時，需隱藏介紹詞與互動內容；僅在「展開」狀態顯示。
29. 前序步驟回顧版面需以單一卡片呈現：每個回顧步驟在同一張卡片中包含步驟說明與互動內容，兩區塊之間以分隔線區隔。
29.1 前序步驟回顧的每個步驟需提供展開/閉合切換 icon（按鈕尺寸需精簡，避免擠壓標題區），預設為閉合；僅在展開時顯示該步驟互動內容區塊。
...
30.2 在進行中課程的前序步驟回顧中，Step4 也需顯示修正後結構樹圖片。

### 6.2.5 Step4（對比修正）
31. Step4（對比修正）學生端流程需拆成三段卡片：先瀏覽同組同學結構樹（可切換同學）、再編修自己的結構樹（自動儲存）、最後進行小組討論。
31.1 Step4「我的結構樹（可編修）」需預設直接展開編修介面，不需額外點擊切換按鈕。
31.2 Step4「我的結構樹（可編修）」載入時需使用該學生目前最新版 `outline`；session 輪詢同步時，若使用者正在本地編修（尚未自動儲存完成）則不得覆蓋本地內容。
31.3 Step4 自己的結構樹編修規則需與 Step3 一致：第一層/第二層固定不可改名、拖曳或刪除；第一層不顯示 `➕`/`➖`，第二層只顯示 `➕`；第三層以下可新增、編輯、拖曳，且無子節點時可刪除。所有允許的新增/刪除/文字編輯皆自動儲存，按 `Enter` 或點擊節點外部完成文字編輯，且不顯示獨立「存檔」按鈕。
31.4 Step4 自己的結構樹區塊需提示：建議先與同學討論後再修改，並明確告知每次編輯都會自動儲存。
...
34.1 確認完成後，「我的結構樹」區塊需以 SVG 視覺化節點圖呈現（與 Step3 完成後及歷史回顧一致），不得顯示 Mermaid 原始程式碼。
35. 僅當 `groupGate[\"4-complete\"]` 包含全組 participants 時，教師端狀態提示顯示「可切換到 Step5」。

### 6.2.6 Step5~Step10（個人步調）
36. Step5（摘要報告）學生端僅顯示摘要卡片，不顯示「互動內容」卡片。
37. Step5 摘要產生並顯示後，學生端需自動推進到 Step6（不需教師手動切換）。
38. Step6 需先顯示「同組結構樹（唯讀）」圖形區，可切換不同同組成員，預設為自己；不得顯示 Mermaid 原始文字。
38.1 學生個人步驟進入 Step6 時，若已有 `draftStep6[username]`，輸入區需自動載入該已存初稿內容。
39. Step6 作文編輯區需為大尺寸輸入框（至少五行高度），並提供「儲存文章」與「AI 修改建議」按鈕。
40. Step6/8 草稿區需顯示清楚保存狀態（#271）：至少涵蓋「正在儲存...」「已自動保存於 HH:mm」「尚有 N 字未保存」「儲存失敗，請稍後再試」；保存狀態顯示在草稿按鈕附近，且不得為了狀態提示增加不必要的頻繁 API 呼叫。
...
44.1 該載入提示需以顯眼樣式呈現（較大字級與高對比底色/邊框），並附簡短說明文字以降低誤解。
45. Step6 觸發「完成文章撰寫，進入下一步驟」後，切換過程需顯示「AI 正在產生步驟 7 分析回饋」提示，回應完成或失敗後自動隱藏。
45.1 該提示需在按鈕所在的互動內容區域可見（按鈕附近顯示），避免使用者誤以為系統當機。
45.2 Step6「完成文章撰寫」前端與後端均需驗證初稿品質（#235）；不符合條件時顯示提示並阻止送出，不得接受空白或隨意填充的內容。驗證規則（寬鬆門檻，防空白非防低品質）：
  - 去除首尾空白後總長度需 >= 100 字元
  - 中文字（CJK U+3400–U+9FFF）需 >= 50 個
  - 去除連續重複模式後長度需 >= 原長度 40%（防 `啊啊啊` / `測試測試`）
  - 不可為單一低品質詞（不知道、隨便、test 等）
  - 後端驗證位於 `POST /api/session/step6/complete`，回傳 `{ error: "draft_insufficient", hint: "..." }` (HTTP 400)
  - 共用驗證函式：`validateDraftContent(draft)` 於 `src/lib/answer-validation.ts`
46. Step7 分析回饋需依 `stepPrompts[\"7\"]` 與該學生 Step6 最終文章內容呼叫 LLM 產生，並寫入 Step7 互動內容（AI 訊息）與個人報告欄位。
47. Step8 編輯區需顯示未儲存字數提示（與 Step6 相同邏輯），並提供「完成潤飾步驟」按鈕；按下後自動切換到 Step9。
47.1 Step8 為潤飾編修流程，不顯示「互動內容」卡片。
47.2 Step9 完成後需自動進入 Step10；Step10 總結報告顯示於互動內容區，結尾需明確包含「整個課程操作結束，請等待老師的下一步指示」。
47.3 Step10 學生端版面需整併：不另顯示「互動內容」卡片與獨立總結卡；改為在 Step10 說明卡中以分隔線後直接顯示總結報告，且使用與互動區相同的 HTML 渲染方式。
47.4 Step10 總結內容顯示後，需額外顯示一張課程結束提醒卡，提示學生等待老師後續指示。
48. 自 Step5 起改為個人步調：同組成員可各自推進 Step5~10，不再要求同組同步切步驟。
48.1 自 Step5 起，學生端「互動內容」僅顯示該學生本人與 AI（及必要的個人系統提示）之訊息，不顯示同組其他學生訊息。
49. Session 需以 `personalSteps[username]` 記錄每位學生在 Step5~10 的個人當前步驟；學生端渲染與 Step5/6/8/9 API 判定均以個人步驟為準。
50. 教師/Admin 監控畫面需顯示 Step5~10 各步驟人數統計（例如 `S5:x / S6:y ... / S10:z`）。
50.1 教師/Admin 在查看課程狀態時，步驟提示於 Step5~10 不得顯示「收齊全班回覆」類訊息；需改為個人步調提示並顯示 Step5~10 人數分布。
50.2 學生進入進行中課程後，需提供可明確返回學生端首頁的操作入口（按鈕文案為「返回學生端課程首頁」）。
...
- 「查看狀態」可查看該課程即時或歷史 session 內容，並可繼續使用：
  ...
  - 小組對話紀錄與個人對話紀錄均採**分步驟卡片**呈現（#236）：
    - 訊息依步驟編號分組，每個步驟顯示為獨立可展開/收起的子卡片
    - 步驟子卡片標頭顯示「步驟 N」，右側提供切換按鈕；**預設全部關閉**（#245）
    - 區塊標題（「小組對話紀錄」/「個人對話紀錄」）右側提供整體展開/關閉按鈕，**整體預設關閉**（#245），點開後才看到分步驟子卡片
    - 切換按鈕統一樣式（#245、#246）：固定 3 字（3em）寬度、深色文字（#1e293b）、置於標題列最右側，文字為「展開」或「關閉」
  - 小組對話紀錄卡片內建**小組選擇器**（#246）：
    - 在課程有 sessions 時即顯示卡片（不再僅在 \`monitorSelected\` 存在時顯示）
    - 卡片內提供下拉選單列出所有小組，選項格式為 `小組 N: 姓名1 (帳號1), 姓名2 (帳號2), ...`
    - 未選擇時顯示「請從上方下拉選單選擇要查看的小組」提示
    - 在課堂儀表板按下「查看對話」會自動：(1) 設定該小組為 \`monitorSelected\` (2) 強制展開「小組對話紀錄」卡片 (3) 平滑捲動到該卡片位置（`scrollIntoView`）
  - 個人對話紀錄卡片內建**學生選擇器**（#249，行為對齊小組對話紀錄）：
    - 在課程有 sessions 時即顯示卡片（不再僅在 \`personalMessages.length > 0\` 才顯示）
    - 卡片內提供下拉選單列出**全課程**所有學生，選項格式為 `小組 N: 姓名 (帳號)`
    - 在「全班加入狀態」表格按「查看」會自動：(1) `loadProgress(sessionId, username)` (2) 展開「個人對話紀錄」卡片 (3) `scrollIntoView` 到該卡片
    - 卡片首次展開但 `selectedProgressUser` 為空時，自動載入第一位學生作為預設
    - 未選擇時顯示提示文字
  - 小組對話紀錄：Step3 完成結構樹（`step3SubmittedOutlines`，各組員）顯示於步驟 2 卡片末尾；Step4 修正後結構樹（`outlines`，若與 Step3 快照不同）顯示於步驟 4 卡片末尾
  - 個人對話紀錄：Step3 完成結構樹顯示於步驟 3 卡片末尾；Step4 修正後結構樹顯示於步驟 4 卡片末尾
  - 結構樹 SVG 不可置於整個對話面板頂部，須依步驟順序與訊息穿插顯示，確保閱讀連貫性
...
- 「全班加入狀態」的人數統計需依「實際已加入課程者」計算，不得以分組 participants 名單直接視為已加入
- 「全班加入狀態」表格欄位（#247，整併原「個人進度表」）：序號、姓名 (帳號)、加入狀態、所在組別、目前進度、發言數、最後發言時間、動作（查看按鈕）。「查看」按鈕呼叫 `loadProgress(sessionId, username)` 載入該學生的個人對話紀錄。原獨立的「個人進度表」卡片已移除。
- 「分組狀態總覽」卡片已移除（#248）：資訊與「全班加入狀態」「課堂儀表板」重複，無存在必要。
- 學習管理操作（課程狀態切換、載入狀態、載入個人進度、切換步驟等）需顯示 processing 提示，完成或失敗後自動結束提示；處理期間僅鎖定相關按鈕或局部區塊（#270），不得因單一操作讓整個學習管理頁不可瀏覽：
  - `開始上課 / 暫停或繼續 / 結束上課 / 重新整理 / 刪除` 僅鎖定該課程列的對應按鈕
  - 課堂儀表板一鍵切換 Step 僅鎖定該小組的推進按鈕
  - `查看對話` 僅鎖定該小組的查看按鈕並於小組紀錄局部載入詳情
  - `查看個人紀錄` 僅鎖定該學生的查看按鈕與個人紀錄區塊
- 學習管理進入分頁時需主動刷新資料；課程清單載入不得被 monitor/session 查詢阻塞
- 僅 admin 可在學習管理課程清單使用「刪除」按鈕；確認刪除後需移除該寫作任務課程全部資料（任務資料、分組資料、學生參與過程/session）。
- 課程狀態檢視需顯示「課堂儀表板」，至少包含小組 session 數、已加入學生數、目前在線學生數、可切下一步小組數、高風險卡關小組數、需留意小組數。
- 「課堂儀表板」「全班加入狀態」「小組對話紀錄」「個人對話紀錄」四個區塊的 h2 標題後方需附加上下文資訊「— 學校 / 班級 / 文章題目」（#258），由 `selectedLearningActivity` 帶入；以較小灰色字體呈現。
- 課堂儀表板列表（#244）：
  - 顯示**所有** sessions（不僅卡關/可推進的小組），按風險優先排序：高風險 → 留意 → 可推進 → 正常
  - 欄位：狀態（badge）、小組（含寫作任務 label）、成員（hover 顯示完整名單）、目前進度（最慢成員的個人步驟）、Step5–10 分布、提醒/步驟切換、動作
  - 整併原「課程狀態內容（即時/歷史）」表格的步驟切換按鈕、查看對話、查看個人進度功能
  - 「目前進度」顯示 `min(personalSteps[member])` 而非 `session.currentStep`，避免 Step 5 後個人步調階段的進度顯示停滯（#244）
- 卡關偵測規則：
  - 若 `getStepAdvanceHint(session).ready` 為 true，標示「可推進」並提供一鍵推進下一步。
  - 若目前 group gate 有未完成組員，且最後事件距今達 10 分鐘以上，標示「高風險」。
  - 若目前 group gate 有未完成組員但未達高風險門檻，標示「留意」。
  - 若無未完成組員但最後事件距今達 10 分鐘以上，標示「留意」並提示查看小組狀況。
  - 若同一學生多次被回答品質檢查拒絕，標示「留意」；達 3 次以上可升為「高風險」，並給老師補強回答完整度的行動建議。
  - Step3 若學生尚未完成且結構樹內容過少（由 `/api/teacher/monitor` 回傳的 `artifactDiagnostics.step3OutlineChars` 判斷），標示「留意」或依閒置時間升為「高風險」。
  - Step3 若學生尚未完成且結構樹長時間沒有更新（由 `artifactDiagnostics.step3OutlineUpdatedAt` 判斷，舊資料缺少時間時視為需留意），標示「留意」並建議老師提醒學生編輯或移動節點。
  - Step6 若學生個人步驟停在 Step6 且初稿字數偏低（由 `artifactDiagnostics.draftStep6Chars` 判斷），標示「留意」或依閒置時間升為「高風險」。
  - 儀表板提醒欄需顯示主要原因、其他原因、需關注學生與至少一項老師可採取的行動建議。
- 課堂儀表板需提供一鍵管理入口：
  - `推進 Step N`：呼叫既有步驟切換流程。
  - `查看對話`：選取該 session 並開啟小組對話紀錄（自動展開卡片並 scrollIntoView）。
  - 「個人進度」按鈕已移除（#247），個人進度功能整併至「全班加入狀態」表格的查看按鈕。
- 在課堂儀表板按下「套用 Step N / 推進 Step N」且切換成功後，對應 session 的按鈕需立即消失（以前端最新 step 狀態即時更新為準）
- 在課堂儀表板觀察狀態中，需自動輪詢 monitor 資料以即時反映小組是否達到可切換步驟條件，避免教師需手動反覆重新整理。輪詢採指數退避（#239）：起始 3 秒，每次 monitor payload 無變化時延遲翻倍（上限 30 秒），偵測到變化（用 sessionId/currentStep/訊息數/最後訊息時間/personalSteps 的雜湊比對）立即重置回 3 秒。閒置課堂網路流量降低 50-70%，活躍課堂仍維持即時反應。

### 課程管理

- 教師端不提供「寫作主題管理」功能（admin 才有）。
- 「課程管理」分頁包含子分頁切換（#253），分為兩個子分頁：
  1. **寫作主題設定**（admin only）：包含「寫作主題管理」卡片
  2. **寫作任務設定**（教師與 admin 皆可見）：包含「增修寫作任務」與「寫作任務管理」兩張卡片
  - 教師端僅顯示「寫作任務設定」按鈕（唯一選項並預設選中）
  - admin 端兩個按鈕都顯示，預設選中「寫作主題設定」
- 「組別管理」獨立卡片已移除（#251）；分組功能整併入「增修寫作任務」。

- 寫作主題管理（admin only）：
  - 「說明」欄位文案為「引導說明」
  - 文體選項需包含：`議論文`、`記敘文`、`說明文`、`抒情文`、`其他`
...
- 增修寫作任務（#251、#252、#254）：
  - 卡片內以兩個 `h3` 子標題分隔區塊：「文章設定」（學校/班級/主題/時長/補充資料）與「小組分配」（小組數量/分組區）
  - 教師：「班級號碼」選單列出該教師所屬學校的所有班級；選項格式為 `${學校} ${班級號碼}`（範例 `xx國中 801`）；ownerTeacherUsername 自動填入該教師
  - admin：先有「學校」選單（系統內所有有學生的學校）→「班級號碼」選單依選定學校過濾；班級選項同樣為 `${學校} ${班級號碼}` 格式
  - admin 選定班級後即時顯示「綁定教師：${name} (${username})」（#254），由該班學生的 ownerTeacherUsername 統計票數最多者推得；找不到時顯示紅字提示
  - 寫作任務儲存時 ownerTeacherUsername 一併寫入，admin 視為「協助該位教師完成設定」，後續執行者為該教師
  - 儲存中 UX（#255）：按下「新增/儲存班級任務」後，按鈕立即 disabled 並顯示「處理中...」字樣；下方出現顯眼藍底處理提示（依步驟切換訊息：寫入任務 → 寫入分組 → 重新整理列表）；儲存成功後改為綠底成功提示（含新任務 ID），5 秒後自動消失。避免使用者誤以為操作未生效而重複送出
  - 「時長 (分鐘)」欄位，輸入正整數
  - 「主題」下拉：顯示 `essayId / title`（僅可選啟用主題；編輯既有任務時可保留既有停用主題）
  - 「補充資料」使用較大文字輸入區
  - 選定班級後，下方即時顯示**分組區**：
    - 「小組數量」欄、「隨機平均分組」、「先建空組」按鈕
    - 「未分配學生」拖拉區與各小組拖拉區
    - 學生數量自動從 users 中該班學生帶入
    - 調整「小組數量」時動態增刪可編輯小組（#252）；增加 → 新組為空；減少 → 被移除小組的成員自動回到「未分配學生」
  - 提交按鈕：新增模式為「新增班級任務」、編輯模式為「儲存班級任務」、編輯時可「取消編輯」
  - 儲存流程（單次點擊）：
    1. POST `/api/admin/openclasses` 取得 activityId
    2. POST `/api/admin/groups` 寫入分組
    3. 觸發 `refreshAll()`
  - 驗證規則：學校（admin）/班級/主題/時長皆必填；分組至少一組；**未分配學生 = 0 才可儲存**，否則顯示提示阻擋

- 寫作任務管理（#251）：
  - 教師：僅顯示該教師所屬學校的任務
  - admin：提供「學校」「班級」兩個下拉（皆可選/不選）
    - 「學校」選單列出**已有任務的學校**
    - 選定學校後「班級」選單列出該校已有任務的班級
  - 排序：由新到舊（依 openClass id `oc-XXX` 序號 desc）
  - 分頁：每頁 10 筆，提供「上一頁」「下一頁」按鈕與「第 N / M 頁」指示
...
- 頁面主標題固定為「系統管理員控制台」。
- admin 需提供獨立子頁「Prompt / LLM 診斷面板」；教師端 `/teacher` 不顯示此面板。
- Prompt / LLM 診斷面板需顯示：
...
- 課程管理：admin 與教師端共用（#251）「寫作主題管理（admin）」、「增修寫作任務」、「寫作任務管理」三張卡片；分組功能整併入「增修寫作任務」（不再有獨立「組別管理」卡片）。詳見 `6.3` 課程管理章節。
