...
13. 步驟名片介紹詞僅在 Step `1/2/3/4/6/8/9` 顯示；Step `5/7` 不顯示。介紹詞內容需讀取 `src/config/step-opening/{step}.txt`，並以 HTML 呈現保留原始換行/段落格式。原先 `任務：xxx / Session: xxx` 顯示需移除。
13.1 進入 Step `1/2/3/4/6/8/9` 時，介紹詞需同步寫入 `session.messages`（`system` 訊息），以便在小組與個人進度紀錄中可追蹤。
