...
- 「查看狀態」可查看該課程即時或歷史 session 內容，並可繼續使用：
  ...
  - 小組對話紀錄：Step3 完成結構樹（`step3SubmittedOutlines`，各組員）需插入在「最後一則 Step2 訊息之後」；Step4 修正後結構樹（`outlines`，若與 Step3 快照不同）需插入在「最後一則 Step4 訊息之後」；若對應步驟尚無訊息，則附加於訊息串末尾
  - 個人對話紀錄：Step3 完成結構樹需插入在「最後一則 Step3 訊息之後」；Step4 修正後結構樹需插入在「最後一則 Step4 訊息之後」；若對應步驟尚無訊息，則附加於訊息串末尾
