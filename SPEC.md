...
13. 步驟名片介紹詞僅在 Step `1/2/3/4/6/8/9` 顯示；Step `5/7` 不顯示。介紹詞內容需讀取 `src/config/step-opening/{step}.txt`，並以 HTML 呈現保留 Markdown 應有格式（至少含標題、粗體/斜體、清單、引用、段落換行）。原先 `任務：xxx / Session: xxx` 顯示需移除。
