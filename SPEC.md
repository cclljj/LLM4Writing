...
7. 進入 session 後顯示課程資訊卡，固定為四行：
  ...
  - 第三行：班級、組別
  - 第四行：組員名單
7.1 題目卡不得顯示「引導說明」與「補充資料」。
7.2 Step1-1 的對話開場需先顯示該課程「引導說明」與「補充資料」，再顯示第一題（子步驟 1-1）；`stepPrompts["1"]` 屬內部 prompt，不得寫入學生可見對話紀錄。
7.3 Step1 互動內容中，「引導說明／補充資料」需以資訊區塊呈現（粗體標題＋內文），不得顯示為「系統提問」；兩者間需有分隔線，之後才開始顯示系統提問。
...
13. Step1-1 開場顯示的「引導說明／補充資料」需以 HTML 方式呈現純文字段落（保留換行與可讀段落結構），避免整段擠成單行。
