...
- 若 OpenAI-compatible `finish_reason` 顯示 token/length 截斷，且不是 Step1/2 結構化 JSON 題目輸出，系統會要求模型續寫並合併文字。
...
- 先檢查最小有效長度（目前實作門檻：至少 20 字）。
- 檢查隨機字串與敷衍詞，明顯無效回答直接拒絕。
- 關聯性採「寬鬆關聯」：命中任一題目核心概念即可視為相關；目前實作支援英文/數字關鍵詞命中，或至少 2 個「非停用」中文字重疊（避免只靠單一功能字誤判相關）。
- 關聯性通過後，若內容偏短且缺少解釋痕跡才回傳補強提示；目前實作為「長度 < 28 且不含解釋標記」時提示補上理由、例子或經驗。解釋標記包含「因為、所以、例如、像是、代表、因此、顯示、說明、比較、影響」。
...
| 步驟 | maxTokens | 備註 |
|---|---|---|
| 1, 2 | 500 | 群組對話短 JSON，`continueOnTruncation: false` |
| 3 | 600 | 結構樹輔導短回覆 |
...
- Step1/2/4：`continuationMaxRounds = 0`。
