...
39. Step6 作文編輯區需為大尺寸輸入框（至少五行高度），並提供「儲存文章」與「請 AI 針對目前內容給我修改建議」按鈕。
40. Step6 需顯示未儲存字數提示（淡色小字）以提醒學生儲存。
41. 點擊 Step6「請 AI 針對目前內容給我修改建議」時，需先自動儲存文章，再以 `stepPrompts[\"6\"]` 作為主提示呼叫 LLM，並將下列格式追加到互動內容卡最下方（可多次累積）：`- 時間：xxxxx`、`- 文章內容：xxxxx`、`- AI 修改建議：xxxxx`；不需新增模擬學生與 AI 對話訊息。
