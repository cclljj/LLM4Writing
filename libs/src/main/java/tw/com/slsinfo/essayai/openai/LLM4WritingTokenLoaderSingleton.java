...
/**
 * Singleton class for llm4writing token loader
 */
public enum LLM4WritingTokenLoaderSingleton {
    INSTANCE;
    ...
    private void init(boolean isLocal) {
        if (isLocal) {
            final String path = IOConstants.UserHomeFolder_Secret.concat(File.separator).concat("essayai")
                    .concat(File.separator).concat("openai_llm4writing.properties");
            ...
            final String path = IOConstants.RemoteHostFolder_Secret.concat(File.separator).concat("openai_llm4writing.properties");
            OpenAITokenManager openAITokenManager
                    = new OpenAITokenManager(
                    new File(path)
            );
