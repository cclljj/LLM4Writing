...
public class ChatPersonalLogModal extends BaseModal<ChatPersonalLogModal.ChatLogData> {
    private static final long serialVersionUID = 1L;
    private static final Logger logger = LogManager.getLogger(ChatPersonalLogModal.class);
    ...
    public ChatPersonalLogModal(String id) {
        super(id);
     ...
     * 設定資料並更新畫面後顯示 Modal
     */
    public void setResponseAndShow(Integer cgid, Integer cid, String groupName, String llmtype, int stageid, AjaxRequestTarget target) {
        this.cgid = cgid;
        this.cid = cid;
        ...
        if (cgid != null) {
            try {
                List<ChatLogs> chatlogs = CDI.current().select(ChatLogsService.class)
                        .get().getChatPersonalLogsbycgid(cgid, cid);
                chatlogListView.setList(chatlogs != null ? chatlogs : List.of());
                logger.debug("載入對話記錄成功: cgid={}, 記錄數={}", cgid,
                        chatlogs != null ? chatlogs.size() : 0);
            } catch (Exception e) {
                logger.debug("載入對話記錄失敗: cgid={}", cgid, e);
    ...
    private void loadStageOptions() {
        if (cgid != null) {
            try {
                ...
                List<Stage> stageIds = service.findAllStages(llmtype, "personal");
    ...
    private void loadChatlogDataByStage() {
        if (cgid != null && selectedStageId != null) {
            try {
                ...
                List<ChatLogs> chatlogs = service.getChatLogsByCgidAndStageId(cgid, cid, selectedStageId, true);
                chatlogListView.setList(chatlogs != null ? chatlogs : List.of());
