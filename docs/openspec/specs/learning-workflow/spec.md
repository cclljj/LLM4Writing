...
#### Scenario: Student joins a term-configured course

- **GIVEN** a student joins an activity with a configured academic year and term
- **WHEN** the join action creates a new session
- **THEN** the session stores that term's `workflowSteps` and `guidedDiscussionSubsteps` snapshots
- **AND** later changes to another term, or to the current term configuration, do not alter the session's child-step sequence
...
- **AND** node placement and connector pathing are generated by a graph-layout engine for stable dense-tree rendering
...
- **GIVEN** stored Step10 text uses legacy escaped line breaks, double-escaped line breaks, JSON string/object wrappers, nested LLM provider response wrappers, content array/text wrappers, markdown marker HTML entities, duplicated markdown heading prefixes, or an outer markdown code fence
