...
- 主要欄位：`id TEXT PRIMARY KEY`、`payload JSONB`、`created_at`、`updated_at`。
- 效能摘要欄位：`current_step`、`workflow`、`activity_id`、`group_id`、`message_count`、`last_message_at`、`participant_count`。
- `saveSession` 仍保存完整 JSONB payload，但必須同步寫入摘要欄位；既有 row 若缺摘要欄位，啟動 migration 需可由 payload 回補。
...
| `src/lib/llm-observability.ts` / `src/lib/llm-stats.ts` | LLM 呼叫時間、串流時間、錯誤分類與 diagnostics 聚合來源 |
...
- 驗證 API 使用者時，server 需先驗證 session token 簽章與期限，再從 user store 重新讀取該帳號並確認角色一致。
...
- payload 僅可包含 `username`、`role`、`exp`、`nonce` 等 session 判斷必要欄位。
- `proxy.ts` 可用 token 內容做頁面導向，但 API/server 權限判定必須再讀 `llm4writing_users` 確認角色仍一致。
...
9. Session payload 尚未拆成 messages/artifacts/reports 多表；目前先以 summary columns 降低監控查詢成本，中期仍可拆表或事件化。
