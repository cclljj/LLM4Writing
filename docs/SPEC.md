...
- 全流程 LLM 呼叫會透過集中解析套用 `max_tokens >= 50000` 的輸出預算下限（含 chat 與 stream 路徑），降低截斷機率。
...
一般流程引擎 `generateAiTextForStep` 會先計算步驟建議預算，再由 LLM client 套用 `max_tokens >= 50000` 的 provider 請求下限：
...
Step-specific streaming / formal report endpoints 會保留原本建議預算與續寫策略，但送出 provider 前同樣套用 `max_tokens >= 50000`：
...
| `/api/session/step10/stream` | 50000 provider floor | 分段生成 | 50000 provider floor | final polish | Step10 總結報告採 outline + section chunking |

預設 LLM client 行為：

- 未指定或指定較小值時，LLM client 送出的 `max_tokens` 皆會提升到至少 `50000`；`timeoutMs = 30_000`、`continueOnTruncation = true`、`continuationMaxRounds = 1`。
