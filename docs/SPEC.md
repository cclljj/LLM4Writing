...
- Step10 總結報告採分段生成（先段落規劃、再逐段產生、必要時再整合潤飾），避免單次長輸出造成 truncation fallback。
- Step10 分段生成由系統負責組合章節標題；LLM outline 回傳的章節標題需先正規化為純文字，移除 Markdown 標題符號、粗體符號、編號與清單符號，且逐段內文若重複輸出同名標題需移除，避免產生 `## ### 標題` 這類雙重 Markdown 標題前綴。
...
- 分段標題需由伺服器以正規化後的純文字標題組合，避免 LLM outline 或 section 內文自行帶入 Markdown 標題造成重複前綴。
