...
| 資料 | 模組 | DB 表 / fallback |
|---|---|---|
| Session | `src/lib/store.ts` | `llm4writing_sessions` + `llm4writing_session_messages` + `llm4writing_session_artifacts` + `llm4writing_session_reports` + `llm4writing_session_events` / `globalThis` memory map |
