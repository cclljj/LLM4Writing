...
- 需提供「一鍵下載全班 PDF」：由後端批次產生每位學生 PDF（檔名規則同個別下載）後打包下載。
- 需提供「一鍵下載全班 JSON」：由後端批次產生每位學生的學習成果 JSON（檔名規則同個別下載）後打包下載；按鈕文字不需標示 ZIP。
...
- 學生成果 JSON schema `student-portfolio-report-v1.1` 需對應 PDF v1.1 的主要資料來源，至少包含課程資訊、學生帳號/姓名、摘要評分、產出時間、完成時間與完整互動歷程。個別「JSON」下載與一鍵下載全班 JSON 必須使用同一個伺服器端學生報告資料組裝流程，不得由前端以個人進度摘要自行產製。`stepArtifacts` 必須列出 Step3 原始架構圖、Step4 討論過程與修正版架構圖、Step5 摘要報告、Step6 初稿、Step7 分析回饋、Step8 潤飾稿、Step10 總結報告；即使某一步沒有內容也需列出並標示 `available=false`。每一個可用的 `stepArtifacts` 成果也必須同步投影到 `timelineMessages`，以 `entryType="artifact"`、`artifactType` 與 `contentFormat` 標示；原始對話為 `entryType="message"`，不得遺失。Step3/Step4 架構圖需同時保留在相容舊欄位 `artifacts`，並提供 Mermaid 原始碼與 Mermaid fenced Markdown，讓架構圖可完整重建。Step1、2、9 的互動內容由 `timelineMessages` 完整保留。輸出內容保留原始文字/Mermaid/Markdown，不轉 HTML；若文字含其他同組成員帳號，需以「有一位組員」遮蔽。
- PDF v1.1 至少需含：
