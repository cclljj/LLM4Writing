...
- Polling 採單一協調流程：無 session 時每 15s `refreshOverview()`；有 session 時以活動期 5s 拉 `/api/session/{id}`，含 ETag/304。若連續 304 或 payload hash 無變化，需採靜止期退避（5s → 10s → 20s → 30s 上限）；偵測到 session 變化後立即恢復 5s。`refreshActivityStatuses()` 維持低頻同步（約每 3 次成功 session poll）。
