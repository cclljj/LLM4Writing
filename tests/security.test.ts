...
test("fallback learning events persist errorCategory for diagnostics (step3/6/7/10)", async () => {
  const { readFileSync } = await import("node:fs");
  const { resolve, dirname } = await import("node:path");
  const { fileURLToPath } = await import("node:url");
  const thisDir = dirname(fileURLToPath(import.meta.url));
  ...
  const step6SuggestRoute = readFileSync(resolve(thisDir, "../app/api/session/step6/suggest/route.ts"), "utf8");
  ...
  assert.ok(step6SuggestRoute.includes("errorCategory"), "step6 fallback events should carry errorCategory");
  ...
  assert.ok(step10Route.includes("step10_stream_route_error"), "step10 route-level fallback should leave a debug trace reason");
});
